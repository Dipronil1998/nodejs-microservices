...
/**
 * Generate formatted HTML for Order Cancellation Email
 * @param {Object} order - Order document
 * @param {string} [reason] - Cancellation reason
 * @param {string} [userName] - Customer name
 * @returns {string} HTML string
 */
export const generateOrderCancelEmailHtml = (order, reason = "Cancelled by user", userName = "Customer") => {
  const itemsHtml = (order.items || [])
    .map(
      (item) => `
        <tr>
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
      ...
      <p>Hello <strong>${order.shippingAddress?.fullName || userName}</strong>,</p>
      <p>Your order <strong>#${order.orderNumber}</strong> has been cancelled successfully.</p>
      
      <div style="background-color: #fff4f4; border-left: 4px solid #d32f2f; padding: 15px; border-radius: 4px; margin: 20px 0;">
        <h4 style="margin: 0 0 8px 0; color: #d32f2f;">Cancellation Details:</h4>
        <p style="margin: 4px 0;"><strong>Order Number:</strong> ${order.orderNumber}</p>
        <p style="margin: 4px 0;"><strong>Reason:</strong> ${reason || order.cancellation?.reason || 'Cancelled by user'}</p>
        <p style="margin: 4px 0;"><strong>Cancelled At:</strong> ${new Date().toLocaleString('en-IN')}</p>
        <p style="margin: 4px 0;"><strong>Payment Status:</strong> ${order.payment?.status || 'CANCELLED'}</p>
      </div>

      ${isRefunded ? `
      <div style="background-color: #e8f5e9; border-left: 4px solid #2e7d32; padding: 12px; border-radius: 4px; margin: 15px 0;">
        <p style="margin: 0; color: #2e7d32; font-size: 14px;">
          <strong>Refund Information:</strong> A refund of <strong>₹${order.pricing?.totalAmount || 0}</strong> has been initiated to your original payment method. It will reflect in your account within 5-7 business days.
        </p>
      </div>` : ''}

      <h4 style="margin: 20px 0 10px 0;">Cancelled Items:</h4>
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
        <thead>
          <tr style="background-color: #f5f5f5;">
            <th style="padding: 10px; text-align: left;">Item</th>
        ...
        <tfoot>
          <tr style="font-size: 15px;">
            <td colspan="2" style="padding: 10px; text-align: right; border-top: 2px solid #333;"><strong>Total Order Amount:</strong></td>
            <td style="padding: 10px; text-align: right; border-top: 2px solid #333;"><strong>₹${order.pricing?.totalAmount || 0}</strong></td>
          </tr>
        </tfoot>
      </table>
      ...
      <p style="color: #666; font-size: 12px; margin-top: 30px; text-align: center;">
        If you did not request this cancellation or have any queries, please contact customer support immediately.
      </p>
    </div>
  `;
};
